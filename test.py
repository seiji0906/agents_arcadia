...
def main():
    ...
    inputs = {
        "messages": [
            HumanMessage(content="下記の[要件]をファイルを実行することで、すぐに実行できるように、してください。例　python ファイル名 。[要件]テキストファイルexample.txtの中で、頻出する単語の上位3位までのランキングを作成する。無視すべき単語（例えば 'the', 'and' など）は除外してください。")
        ]
    }
